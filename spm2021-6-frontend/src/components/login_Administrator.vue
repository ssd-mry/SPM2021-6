<template>
  <div class="card">
    <div class="form">
      ...
      <el-input v-model="item.input" placeholder="请输入账号" width="100px" class="first"></el-input>
    </div>
    <div class="form">
      <img src="../assets/password.png" height="30px" width="30px" class="me">
      <el-input placeholder="请输入密码" v-model="item.password" show-password class="second"></el-input>
    </div>
    <div>
      <el-row>
    ...
    methods: {
      handleClick(Path) {
        ...
        if (this.item.input === '' || this.item.password === '') {
          ...
          this.$axios({
            method: 'post',
            url: 'http://localhost:8080/user/superLogin',
            ...
            if(res.data === 1){
              ...
              this.$router.push('/menu_Administrator')
            }
          }).catch(error => {
            alert(error)
            alert('账号或密码错误')
